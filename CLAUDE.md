...
- Токен и логин — из env (`.env`), не хардкодить.
...
Не бить по боевому Direct API в тестах и при разработке. Секреты — только в `.env`
(есть `.env.example`), в репозиторий не коммитить.
